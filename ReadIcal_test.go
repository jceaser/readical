...
func TestStrToNumber(t *testing.T) {
	tester := func (given string, fallback int, expected int, msg string){
		...
		if expected!=actual {
			report := fmt.Sprintf("Failed %s test, %s!=%d", msg, given, expected)
			t.Error(report)
		}
	}
...
func TestDateStrToObj(t *testing.T) {
    ...
    expected = time.Date(2021, 5, 30, 0, 0, 0, 0, time.UTC)
    actual = dateStrToObj("20210530", -4)
    if expected != actual {
        t.Errorf("dateStrToObj-date does not match: [%s] vs [%s].", expected, actual)
    }
}

func TestWork(t *testing.T) {
	expected := `
## <i class="fas fa-user-friends"></i> June 02, 2021: 07:00 PM - No Meeting

There is no meeting on this day

* When: 2021-06-02 07:00 PM to 2021-06-02 08:00 PM
...
`
    ...
    deleteFile("/tmp/calendar-2021-05-02-to-2021-06-03.md")
